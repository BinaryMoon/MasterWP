	...
	$.fn.elementalSlides = function( options ) {
		...
		return this.each( function() {
			...
			if ( 0 === nav.length ) {
				...
				nav = $( '<nav></nav>' );
				nav.attr( 'aria-label', i18n.slide_controls_label );
				$this.prepend( nav );
			...
			articles.each(
				function() {
					...
					var tab = $( '<button type="button" data-slide="' + slide_count + '" class="tab"><span class="screen-reader-text">' + i18n.slide_number.replace( '#', slide_count ) + '</span></button>' );
					nav.append( tab );
			...
			if ( options.nav_arrows ) {
				...
				var arrow_next = $( '<button type="button" class="arrow arrow-next">' + i18n.slide_next + '</button>' );
				var arrow_prev = $( '<button type="button" class="arrow arrow-prev">' + i18n.slide_prev + '</button>' );
